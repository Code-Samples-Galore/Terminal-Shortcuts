...
if ! should_exclude "sc" 2>/dev/null; then
  sc() {
    ...
    echo "  strconv     - Convert strings/data between encodings (hex, base64, binary)"
    echo "  unitconv    - Convert between different units (length, weight, temperature, volume, area, time, data)"
    ...
    echo "  less        - Enhanced less (less -RMNi --use-color)"
    ...
    echo "  preq        - Analyze Python project dependencies and environment information"
    echo "  pytestcov   - Run Python tests with coverage analysis using pytest"
    echo "  pfmt        - Format and lint Python code using black, isort, and flake8"
    ...
    echo "  list_functions - List all defined functions in current shell session"
